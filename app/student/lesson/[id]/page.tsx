...
export default function LessonPlayer() {
  ...
  const updateProgress = async (slideIndex: number, isEnd = false) => {
  ...
  useEffect(() => {
      if (slides.length > 0 && userId) {
          updateProgress(currentSlide, false);
      }
  }, [currentSlide, slides, userId]);
    ...
    fetch(`/api/lesson/${params.id}`)
      ...
      .then(data => {
        if (data.success) {
           ...
           // Gemini now returns { title, content } for slides. Normalize:
           rawSlides = rawSlides.map((s: any, i: number) => ({
              title: s.title || `Алхам ${i+1}`,
              content: s.content || (typeof s === 'string' ? s : "Агуулга байхгүй"),
              image: ["🔬", "🧬", "🌍", "📚", "🚀", "💡"][i % 6]
           }));
  ...
  const handleNextSlide = async () => {
    if (showQuiz) {
      ...
      if (quizAnswered) {
        if (currentSlide < slides.length - 1) {
          ...
          await updateProgress(currentSlide, true);
    ...
    const isQuizTime = quizzes.length > 0 && currentQuizIndex < quizzes.length && (currentSlide + 1) % 2 === 0;
    ...
    <div className="h-screen flex flex-col bg-[#070914] text-slate-100 font-sans selection:bg-[#00f5ff]/30">
      ...
      <header className="h-14 border-b border-[#00f5ff]/20 bg-[#0b0a1a]/80 backdrop-blur-md flex items-center justify-between px-6 z-10">
        ...
        {!loading && slides.length > 0 && (
          <div className="flex items-center gap-4 text-sm font-medium">
            <span className="text-[#00f5ff]">Алхам {currentSlide + 1} / {slides.length}</span>
          </div>
        )}
      </header>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col relative isolate">
          <div className="absolute top-[20%] left-[-10%] w-[50%] h-[50%] bg-[#00f5ff]/5 blur-[120px] rounded-full pointer-events-none z-0" />
          
          <main className="flex-1 overflow-y-auto flex flex-col pt-10 z-10">
            <div className="flex-1 flex flex-col items-center justify-center p-8 w-full my-auto">
              {loading ? (
                ...
                <div className="max-w-4xl w-full animate-[rise_0.5s_ease-out]">
                  ...
                  <div className="glow-card p-10 bg-[#0b0a1a]/95 border border-[#00f5ff]/30 text-slate-200 leading-relaxed shadow-[0_0_30px_rgba(0,245,255,0.05)]">
                    <MarkdownRenderer 
          ...
          {!loading && slides.length > 0 && (
            <div className="h-24 shrink-0 border-t border-[#00f5ff]/20 bg-[#0b0a1a]/95 flex items-center justify-between px-8 w-full z-20 backdrop-blur-xl">
              <button 
              ...
              <button 
                 className={`px-8 py-3 flex items-center gap-2 font-bold rounded-xl transition-all disabled:opacity-50 disabled:grayscale ${showQuiz && quizAnswered !== null ? (quizAnswered ? 'bg-emerald-500 hover:bg-emerald-600 text-white shadow-[0_0_15px_rgba(16,185,129,0.4)]' : 'bg-white/20 hover:bg-white/30 text-white') : 'button-gradient'}`} 
                 disabled={showQuiz && quizAnswered === null} 
                 onClick={handleNextSlide}
              >
                {showQuiz ? 'Үргэлжлүүлэх' : (currentSlide === slides.length - 1 ? 'Дуусгах' : 'Дараах')} <ArrowRight className="w-4 h-4" />
              </button>
            </div>
          )}
        </div>
